"""Qwen3-32B 对比实验：few-shot K=20 + 语义描述 K=20（完整数据）"""
...
MODEL_PATH = "/mnt/parscratch/users/acp25bl/models/Qwen3-32B"
DATA_PATH = "/mnt/parscratch/users/acp25bl/processed/pulsedb_features_full.npz"
OUTPUT_PATH = "/users/acp25bl/bishe/llm_bp_32b_results.json"
...
def main():
    print("=" * 70)
    print("Qwen3-32B comparison: numeric K=20 vs semantic K=20")
    print("=" * 70)
    ...
    model.eval()
    print("[2/3] Model loaded (32B)", flush=True)
    ...
    for si, subj in enumerate(test_subjs):
        ...
        for ci in test_idx:
            ...
            ps1, pd1 = parse_bp(r1)
            numeric_results.append({"subj": subj, "tsbp": float(y_sbp[ci]),
                                   "tdbp": float(y_dbp[ci]), "psbp": ps1, "pdbp": pd1})

            r2 = generate(model, tokenizer, build_semantic_prompt(sem, calib_sem))
            ps2, pd2 = parse_bp(r2)
            semantic_results.append({"subj": subj, "tsbp": float(y_sbp[ci]),
                                     "tdbp": float(y_dbp[ci]), "psbp": ps2, "pdbp": pd2})
        print(f"  subject {si+1}/{len(test_subjs)} done", flush=True)

    summary = {}
    for name, res in [("numeric", numeric_results), ("semantic", semantic_results)]:
        valid = [r for r in res if r["psbp"] is not None]
        if valid:
            summary[name] = {
                "n": len(valid),
            ...
            print(f"  {name} K=20: SBP {summary[name]['sbp_mae']:.1f}, "
                  f"DBP {summary[name]['dbp_mae']:.1f} (n={len(valid)})", flush=True)

    with open(OUTPUT_PATH, "w") as f:
        json.dump({"model": "Qwen3-32B", "K": K, "summary": summary,
